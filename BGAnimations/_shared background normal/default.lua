	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.Sprite{
				...
				NewColorCommand=function(self) self:linear(delay):diffuse(GetHexColor(index+1)):diffusealpha(0.3) end;
			},

			Def.Sprite{
				Texture=file,
				InitCommand=cmd(zoom, 1.3; x,-050; y,040; z,-100; customtexturerect,0,0,1,1; texcoordvelocity,0.04,.01 ),
				NewColorCommand=function(self) self:linear(delay):diffuse(GetHexColor(index+1)):diffusealpha(0.2) end;
			},
