...
for key, item in pairs(PaneItems) do
	...
	pd[#pd+1] = Def.ActorFrame{
		...
		Name=key,
		OnCommand=function(self) self:x(-_screen.w/20; y,6 ) end;
...
pd[#pd+1] = Def.BitmapText{
	...
	InitCommand=function(self) self:x(highscoreX):y(156):zoom(zoom_factor):diffuse(Color.Black):halign(1) end;
}
...
pd[#pd+1] = Def.BitmapText{
	...
	InitCommand=function(self) self:x(highscorenameX):y(156):zoom(zoom_factor):diffuse(Color.Black):halign(0):maxwidth(80) end;
...
pd[#pd+1] = Def.BitmapText{
	...
	InitCommand=function(self) self:x(highscoreX):y(176):zoom(zoom_factor):diffuse(Color.Black):halign(1) end;
...
pd[#pd+1] = Def.BitmapText{
	...
	InitCommand=function(self) self:x(highscorenameX):y(176):zoom(zoom_factor):diffuse(Color.Black):halign(0):maxwidth(80) end;
}
