...
for i=1,NumWheelItems-1 do
	...
	t[#t+1] = Def.Quad	{
		InitCommand=function(self)
			self:xy(_screen.cx+_screen.w/4, 9 + (_screen.h/(NumWheelItems+1))*i):zoomto(_screen.w/2, (_screen.h/(NumWheelItems))/2); 
			self:diffuse(ThemePrefs.Get("RainbowMode") and Color.White or Color.Black)
		end;
		OnCommand=function(self)
			self:sleep(i*0.057):linear(0.125):cropbottom(1):diffusealpha(0.5):queuecommand("Hide")
		end;
	...
	t[#t+1] = Def.Quad	{
		InitCommand=function(self)
			self:xy(_screen.cx+_screen.w/4, 25 + (_screen.h/(NumWheelItems+1))*i):zoomto(_screen.w/2, (_screen.h/(NumWheelItems))/2); 
			self:diffuse(ThemePrefs.Get("RainbowMode") and Color.White or Color.Black)
		end;
		OnCommand=function(self)
			self:sleep(i*0.057):linear(0.125):croptop(1):diffusealpha(0.5):queuecommand("Hide")
		end;
		HideCommand=function(self) self:visible(false) end
	}
end
